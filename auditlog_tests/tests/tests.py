...
from auditlog_tests.models import SimpleModel, AltPrimaryKeyModel, UUIDPrimaryKeyModel, \
    PolymorphicModel, RelatedModel, ManyRelatedModel, SimpleIncludeModel, SimpleExcludeModel, User
...
class TestSimpleModel(BaseTest):
    ...
    def create_obj(self, db: Session):
        obj = SimpleModel(text="I am not difficult.", boolean=False)
        db.add(obj)
        db.commit()
        db.refresh(obj)
        return obj
...
class TestAltPrimaryKeyModel(BaseTest):
    model = AltPrimaryKeyModel

    def create_obj(self, db: Session) -> Any:
        obj = AltPrimaryKeyModel(key=str(datetime.datetime.now()), text='I am strange.')
        db.add(obj)
        db.commit()
        db.refresh(obj)
        return obj
...
class TestUUIDPrimaryKeyModel(BaseTest):
    ...
    def create_obj(self, db: Session) -> Any:
        obj = UUIDPrimaryKeyModel(text='I am strange.')
        db.add(obj)
        db.commit()
        db.refresh(obj)
        return obj
...
class TestPolymorphicModel(BaseTest):
    ...
    def create_obj(self, db: Session) -> Any:
        obj = PolymorphicModel(text='I am not what you think.')
        db.add(obj)
        db.commit()
        db.refresh(obj)
        return obj
...
class TestRelatedModel:
    def test_related(self, db: Session, mock_save) -> Any:
        simple = SimpleModel(text='simple')
        db.add(simple)
        db.flush()
        obj = RelatedModel(related_id=simple.id)
        db.add(obj)
        db.commit()
        assert mock_save.call_count == 2
    ...
    def test_many_related(self, db: Session, mock_save) -> Any:
        ...
        db.add(simple)
        db.flush()
        obj = ManyRelatedModel(text='related')
...
class TestSimpleIncludeModel:
    def test_register_include_fields(self, db: Session, mock_save):
        obj = SimpleIncludeModel(label='Include model', text='Looong text')
        db.add(obj)
        db.commit()
        db.refresh(obj)
...
class TestSimpleExcludeModel:
    def test_register_include_fields(self, db: Session, mock_save):
        obj = SimpleExcludeModel(label='Exclude model', text='Looong text')
        db.add(obj)
        db.commit()
        db.refresh(obj)
...
class UnregisterTest:
    ...
    def unregistered_obj(self, db: Session):
        ...
        obj = SimpleModel(text="No history")
        db.add(obj)
        db.commit()
        db.refresh(obj)
        return obj

    def test_unregister_create(self, unregistered_obj: SimpleModel, mock_save):
        assert mock_save.call_count == 0

    def test_unregister_update(self, db: Session, unregistered_obj: SimpleModel, mock_save):
    ...
    def test_unregister_delete(self, db: Session, unregistered_obj: SimpleModel, mock_save):
...
class TestActor:
    ...
    @pytest.fixture(scope="function")
    def db_actor(self, db: Session) -> Session:
        user = User(email='mail@mail.com')
        db.add(user)
        db.commit()
        db.refresh(user)
        set_user(db, user)
    ...
    @pytest.fixture(scope="function")
    def obj(self, db_actor: (Session, User)) -> SimpleModel:
        db, _ = db_actor
        obj = SimpleModel(text="I am not difficult.", boolean=False)
        db.add(obj)
        db.commit()
    ...
    def test_create(self, db_actor: (Session, User), obj: SimpleModel, mock_save):
        db, user = db_actor
        assert mock_save.call_count == 2
        kwargs = mock_save.call_args.args[0]
    ...
    def test_update(self, db_actor: (Session, User), obj: SimpleModel, mock_save):
        db, user = db_actor
        obj.text = 'new text'
        db.add(obj)
        db.commit()
    ...
    def test_delete(self, db_actor: (Session, User), obj: SimpleModel, mock_save):
        db, user = db_actor
        db.delete(obj)
        db.commit()
        assert mock_save.call_count == 3
...
class TestRemoteAddr:
    ...
    @pytest.fixture(scope="function")
    def obj(self, db: Session, remote_addr) -> SimpleModel:
        obj = SimpleModel(text="I am not difficult.", boolean=False)
        db.add(obj)
        db.commit()
        db.refresh(obj)
    ...
    def test_create(self, obj: SimpleModel, mock_save):
        assert mock_save.call_count == 1
        kwargs = mock_save.call_args.args[0]
        assert kwargs['remote_addr'] == self.REMOTE_ADDR

    def test_update(self, db: Session, obj: SimpleModel, mock_save):
        obj.text = 'new text'
        db.add(obj)
        db.commit()
        assert mock_save.call_count == 2
    ...
    def test_delete(self, db: Session, obj: SimpleModel, mock_save):
        db.delete(obj)
        db.commit()
        assert mock_save.call_count == 2
        kwargs = mock_save.call_args.args[0]
